...
KeiPix 是一个原生 Swift Pixiv 客户端，当前以 macOS 26+ 为主平台，并已具备正式 iPadOS 26+ XcodeGen target 与 Simulator 验证路线。它不是 Pixiv 官方项目；目标是在 Apple 平台上用更贴近系统控件的方式做好浏览、阅读、下载、本地库和 QA。
...
最新本地基线为 **302 个测试 / 52 个测试套件**，覆盖模型、解析、URL 路由、下载、阅读模式、视觉 QA manifest、Runtime Readiness、平台边界和 native bridge 守卫。
