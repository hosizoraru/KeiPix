...
struct NativeBoundaryTests {
    ...
    func iPadRootContentDoesNotInheritMacOSWindowSizing() throws {
        ...
        #expect(app.contains("minWidth: MainWindowSizing.minimumWidth(sidebarVisible: false)"))
        #expect(app.contains("minWidth: MainWindowSizing.minimumWidth(sidebarVisible: true)") == false)
    ...
    func simulatorRunScriptsCoverMobileSchemes() throws {
        ...
        let os27DeviceHubStatusURL = root.appending(path: "script/os27/devicehub_status.sh")
        ...
        let os27DeviceHubStatus = try String(contentsOf: os27DeviceHubStatusURL, encoding: .utf8)
        ...
        #expect(FileManager.default.fileExists(atPath: os27DeviceHubStatusURL.path(percentEncoded: false)))
        ...
        #expect(os27DeviceHubStatus.contains("DeviceHub.app"))
        #expect(os27DeviceHubStatus.contains("DeviceKit.framework"))
        #expect(os27DeviceHubStatus.contains("CoreDevice.framework"))
        #expect(os27DeviceHubStatus.contains("xcrun devicectl list devices --json-output"))
        #expect(os27DeviceHubStatus.contains("xcrun devicectl device info displays"))
    ...
    func iPadFeedRootDispatchesNonArtworkRoutes() throws {
        ...
        #expect(contentView.contains("private func galleryLayoutToolbarItem(showsSidebarToggle: Bool) -> some ToolbarContent"))
        #expect(contentView.contains("systemImage: store.galleryLayoutMode.systemImage"))
        #expect(contentView.contains("accessibilityLabel: L10n.galleryLayout"))
        ...
        #expect(contentView.contains("@State private var preferredLandscapeCompactColumn: NavigationSplitViewColumn = .detail"))
        #expect(contentView.contains("NavigationSplitView(\n            columnVisibility: $splitColumnVisibility,\n            preferredCompactColumn: $preferredLandscapeCompactColumn"))
        ...
        #expect(contentView.contains("private func sidebarToggleButton(title: String) -> some View"))
        #expect(contentView.contains("Label(title, systemImage: \"sidebar.leading\")"))
        ...
        #expect(contentView.contains("private func routeMenuItems(for section: MobileRouteMenuSection) -> [NativeToolbarMenuItem]"))
        #expect(contentView.contains("case .submenu(let systemImage):"))
        #expect(contentView.contains("subtitle: selectedRouteTitle(in: section)"))
        #expect(contentView.contains("presentation: .singleSelection"))
        #expect(contentView.contains("NativeToolbarMenuButton("))
        ...
        #expect(mobileBottomTabConfiguration.contains("enum MobileRouteMenuSectionPresentation"))
        #expect(mobileBottomTabConfiguration.contains("presentation: .submenu(systemImage: \"chart.bar\")"))
        #expect(mobileBottomTabConfiguration.contains("presentation: .submenu(systemImage: \"chart.bar.doc.horizontal\")"))
        ...
        #expect(contentView.contains("} else {\n            ToolbarItem(placement: .primaryAction) {\n                appControlsToolbarButton(showsSidebarToggle: showsSidebarToggle)"))
        #expect(contentView.contains("private func appControlsToolbarButton(showsSidebarToggle: Bool) -> some View"))
        ...
        #expect(nativeToolbarMenu.contains("let cacheKey: String?"))
        #expect(nativeToolbarMenu.contains("lhs.cacheKey == rhs.cacheKey"))
        #expect(nativeToolbarMenu.contains("UIMenu.Identifier(cacheKey)"))
        #expect(nativeToolbarMenu.contains("singleSelectionSubmenu<Option>("))
        #expect(nativeToolbarMenu.contains("selectedTitle: String"))
        #expect(nativeToolbarMenu.contains("selectedOption: Option"))
        ...
        #expect(spotlightView.contains("loadMoreIfNeeded(after: article)"))
        ...
        #expect(spotlightView.contains("@Environment(\\.horizontalSizeClass) private var horizontalSizeClass") == false)
        #expect(spotlightView.contains("private var usesCompactSpotlightChrome: Bool") == false)
        #expect(spotlightView.contains("private var collectionModePicker: some View"))
        #expect(spotlightView.contains("compactCollectionMenu") == false)
        #expect(spotlightView.contains("fixedCollectionMode: SpotlightArticleCollectionMode? = nil"))
        #expect(spotlightView.contains("ToolbarItem(placement: .primaryAction) {\n                    viewOptionsMenu"))
        #expect(spotlightView.contains("ToolbarItem(placement: .secondaryAction) {\n                    viewOptionsMenu") == false)
        #expect(spotlightView.contains("#if os(iOS)\n        NativeToolbarMenuButton("))
        #expect(spotlightView.contains("private var nativeViewOptionsMenu: NativeToolbarMenu"))
        #expect(spotlightView.contains("cacheKey: nativeViewOptionsMenuCacheKey"))
        #expect(spotlightView.contains("NativeToolbarMenuItem.singleSelectionSubmenu("))
        #expect(spotlightView.contains("private func handleNativeViewOptionsAction(_ id: String)"))
        #expect(spotlightView.contains("private enum SpotlightViewOptionsAction"))
        #expect(spotlightView.contains("private enum SpotlightArticleCardPresentation"))
        ...
        #expect(spotlightView.contains("GeometryReader { proxy in"))
        #expect(spotlightView.contains("presentation: spotlightArticleCardPresentation(containerWidth: proxy.size.width)"))
        #expect(spotlightView.contains("private func spotlightArticleCardPresentation(containerWidth: CGFloat) -> SpotlightArticleCardPresentation"))
        #expect(spotlightView.contains("private func estimatedArticleColumnCount(containerWidth: CGFloat) -> Int"))
        #expect(spotlightView.contains("UIDevice.current.userInterfaceIdiom == .phone"))
        #expect(spotlightView.contains("return columnCount == 1 ? .immersiveCover : .standard"))
        #expect(spotlightView.contains("return columnCount <= 2 ? .immersiveCover : .standard"))
        #expect(spotlightView.contains("isRead: store.isSpotlightArticleRead(article)"))
        #expect(spotlightView.contains("let isRead: Bool"))
        #expect(spotlightView.contains("let presentation: SpotlightArticleCardPresentation"))
        #expect(spotlightView.contains("if presentation == .immersiveCover"))
        #expect(spotlightView.contains("private var immersiveCoverLayout: some View"))
        #expect(spotlightView.contains("private var immersiveMetadataOverlay: some View"))
        #expect(spotlightView.contains("toggleRead: {"))
        #expect(spotlightView.contains("private func toggleRead(_ article: PixivSpotlightArticle)"))
        #expect(spotlightView.contains("store.markSpotlightArticleRead(article)"))
        #expect(spotlightView.contains("store.markSpotlightArticleUnread(article)"))
        #expect(spotlightView.contains("L10n.readArticle"))
        #expect(spotlightView.contains("L10n.markArticleUnread"))
        #expect(spotlightView.contains(".nativeBottomTabContentSurface()"))
        #expect(spotlightView.contains("private var articleThumbnail: some View"))
        #expect(spotlightView.contains(".overlay(alignment: .topTrailing)"))
        #expect(spotlightView.contains("private var articleStateOverlayBadges: some View"))
        #expect(spotlightView.contains("title: L10n.readArticle"))
        #expect(spotlightView.contains("systemImage: \"checkmark.circle.fill\""))
        #expect(spotlightView.contains("articleStateFooter") == false)
        ...
        #expect(spotlightView.contains("Label(L10n.openArticle, systemImage: \"newspaper\")") == false)
        #expect(spotlightView.contains("article.publishDate.formatted(date: .abbreviated") == false)
        #expect(spotlightView.contains("Section(L10n.pixivisionDisplay)"))
        #expect(spotlightView.contains("pixivisionCollectionMenu"))
        #expect(spotlightView.contains("pixivisionCategoryMenu"))
        #expect(spotlightView.contains("pixivisionLayoutMenu"))
        #expect(spotlightView.contains("private func pixivisionPickerMenu<SelectionValue: Hashable, Options: View>("))
        #expect(spotlightView.contains("Text(currentValueTitle)"))
        #expect(spotlightView.contains(".menuOrder(.fixed)"))
        #expect(spotlightView.contains("if usesCompactSpotlightChrome == false, fixedCollectionMode == nil") == false)
        #expect(spotlightView.contains("if usesCompactSpotlightChrome, store.session != nil, fixedCollectionMode == nil") == false)
        #expect(spotlightView.contains("if collectionMode.supportsCategoryFilter, usesCompactSpotlightChrome == false") == false)
        #expect(spotlightView.contains("ToolbarItem(placement: .principal)") == false)
        #expect(spotlightView.contains(".accessibilityLabel(\"\\(L10n.spotlightCollection): \\(collectionMode.title)\"") == false)
    ...
    func iOSAppControlsAndBottomTabCustomizationKeepCriticalEntriesReachable() throws {
        ...
        #expect(contentView.contains("setArtworkImageQualityTier("))
        #expect(contentView.contains("artworkImageQualityMenuItem"))
        #expect(contentView.contains("L10n.feedPreviewQuality") == false)
        #expect(contentView.contains("L10n.illustDetailQuality") == false)
        #expect(contentView.contains("L10n.mangaDetailQuality") == false)
    ...
    func compactSearchIsOptInInsteadOfPersistentEverywhere() throws {
        ...
        let phoneFeedFilterLayout = try String(
        ...
        #expect(iPadContentView.contains("chromeMaterialMode: store.chromeMaterialMode"))
        #expect(iPadContentView.contains(".mobileToolbarChromeMaterial(syncID: \"feed|\\(showsSidebarToggle)|\\(store.selectedRoute.rawValue)\")"))
        #expect(iPadContentView.contains(".mobileToolbarChromeMaterial(syncID: \"library\")"))
        #expect(iPadContentView.contains(".mobileToolbarChromeMaterial(syncID: \"settings\")"))
        #expect(iPadContentView.contains(".mobileToolbarChromeMaterial(syncID: \"settings|landscape\")"))
        ...
        #expect(iPadContentView.contains("@State private var isPhoneFeedAtContentStart") == false)
        #expect(iPadContentView.contains("compactTabBarGeometry") == false)
        #expect(iPadContentView.contains("isPhoneFeedAtContentStart = event.isAtContentStart") == false)
        #expect(iPadContentView.contains("handlePhoneGalleryScrollEvent") == false)
        #expect(iPadContentView.contains("galleryScrollDirectionHandler") == false)
        #expect(iPadContentView.contains("private var phoneHasActiveFeedFilter: Bool"))
        #expect(iPadContentView.contains("private var phoneSupportsClientFeedFilter: Bool"))
        #expect(iPadContentView.contains("if store.selectedRoute == .downloads {\n            return true\n        }"))
        #expect(iPadContentView.contains("store.selectedRoute.usesArtworkFeed\n            || (store.selectedRoute.usesNovelFeed && store.selectedRoute != .novelWatchlist)\n            || store.selectedRoute == .pixivActivity"))
        #expect(iPadContentView.contains("store.downloads.downloadQueueFilter != .all"))
        #expect(iPadContentView.contains("store.downloads.items.filter(store.downloads.downloadQueueFilter.includes).count"))
        #expect(iPadContentView.contains("return store.downloads.filteredItems.count"))
        #expect(iPadContentView.contains("return store.pixivActivityItems.count"))
        #expect(iPadContentView.contains("return store.pixivActivityVisibleItems.count"))
        #expect(iPadContentView.contains("private var phoneFeedFilterPlaceholder: String"))
        #expect(iPadContentView.contains("return L10n.filterDownloads"))
        #expect(iPadContentView.contains("return L10n.filterActivity"))
        #expect(iPadContentView.contains("return L10n.filterNovels"))
        #expect(iPadContentView.contains("return L10n.filterResults"))
        #expect(iPadContentView.contains("isPhoneCollapsedFeedFilterVisible") == false)
        #expect(iPadContentView.contains("onGalleryScrollDirectionChange: nil"))
        ...
        #expect(tabBarBridge.contains("private var glassContainerView: UIView?"))
        #expect(tabBarBridge.contains("private var pillView: UIVisualEffectView?"))
        #expect(tabBarBridge.contains("private var panelView: UIVisualEffectView?"))
        #expect(tabBarBridge.contains("private var isPanelPresented = false"))
        #expect(tabBarBridge.contains("private weak var observedScrollView: UIScrollView?") == false)
        #expect(tabBarBridge.contains("private var scrollObservation: NSKeyValueObservation?") == false)
        #expect(tabBarBridge.contains("private var layoutTimer: Timer?") == false)
        #expect(tabBarBridge.contains("let effect = UIGlassContainerEffect()"))
        #expect(tabBarBridge.contains("let glassEffect = UIGlassEffect(style: .regular)"))
        #expect(tabBarBridge.contains("glassEffect.isInteractive = isInteractive"))
        #expect(tabBarBridge.contains("overlayView = UIVisualEffectView(effect: glassEffect)"))
        #expect(tabBarBridge.contains("PassThroughVisualEffectView(effect: effect)"))
        #expect(tabBarBridge.contains("private final class PassThroughVisualEffectView: UIVisualEffectView"))
        #expect(tabBarBridge.contains("UIBlurEffect(style: .systemChromeMaterial)"))
        #expect(tabBarBridge.contains("UIBlurEffect(style: .systemUltraThinMaterial)") == false)
        #expect(tabBarBridge.contains("let field = UITextField(frame: .zero)"))
        #expect(tabBarBridge.contains("field.accessibilityLabel = L10n.filterArtworks") == false)
        #expect(tabBarBridge.contains("filterField?.accessibilityLabel = placeholder"))
        ...
        #expect(tabBarBridge.contains("closeButton.addTarget(self, action: #selector(hideFilterPanel), for: .touchUpInside)"))
        #expect(tabBarBridge.contains("pillControl.addTarget(self, action: #selector(showFilterPanel), for: .touchUpInside)"))
        #expect(tabBarBridge.contains("let pillStack = UIStackView(arrangedSubviews: [pillIconView, pillLabel])"))
        #expect(tabBarBridge.contains("pillStack.centerXAnchor.constraint(equalTo: pillControl.centerXAnchor)"))
        #expect(tabBarBridge.contains("pillStack.centerYAnchor.constraint(equalTo: pillControl.centerYAnchor)"))
        #expect(tabBarBridge.contains("pillLabel.minimumScaleFactor = 0.86"))
        #expect(tabBarBridge.contains("filterField?.resignFirstResponder()"))
        let hideFilterPanelBody = tabBarBridge
            .components(separatedBy: "@objc private func hideFilterPanel()")
            .dropFirst()
            .first
            .map { $0.components(separatedBy: "func textFieldShouldReturn").first ?? "" } ?? ""
        #expect(hideFilterPanelBody.contains("isPanelPresented = false"))
        #expect(hideFilterPanelBody.contains("filterField?.resignFirstResponder()"))
        #expect(hideFilterPanelBody.contains("applyOverlay()"))
        #expect(hideFilterPanelBody.contains("text.wrappedValue") == false)
        #expect(tabBarBridge.contains("L10n.feedFilter"))
        #expect(tabBarBridge.contains("L10n.filtering"))
        ...
        #expect(iPadContentView.contains("isPhoneFeedAtContentStart ? \"content-start\" : \"content-scrolled\"") == false)
        ...
        #expect(tabBarBridge.contains("let placeholder: String"))
        ...
        #expect(tabBarBridge.contains("tabBarController.view.addSubview(glassContainerView)"))
        #expect(tabBarBridge.contains("glassContentView(in: glassContainerView).addSubview(pillView)"))
        #expect(tabBarBridge.contains("glassContentView(in: glassContainerView).addSubview(panelView)"))
        #expect(tabBarBridge.contains("tabBarController.view.addSubview(pillView)") == false)
        #expect(tabBarBridge.contains("tabBarController.view.addSubview(panelView)") == false)
        #expect(tabBarBridge.contains("tabBarController.view.bringSubviewToFront(glassContainerView)"))
        #expect(tabBarBridge.contains("tabBarController.view.bringSubviewToFront(pillView)") == false)
        #expect(tabBarBridge.contains("tabBarController.view.bringSubviewToFront(panelView)") == false)
        #expect(tabBarBridge.contains("syncObservedScrollView(in: tabBarController)") == false)
        ...
        #expect(tabBarBridge.contains("selectedContentIsAtStart") == false)
        ...
        #expect(tabBarBridge.contains("scrollView?.observe(\\.contentOffset, options: [.new])") == false)
        #expect(tabBarBridge.contains("private func selectedContentScrollView(in tabBarController: UITabBarController) -> UIScrollView?") == false)
        #expect(tabBarBridge.contains("allowShortContent: true"))
        #expect(tabBarBridge.contains("private var hasActiveFilter: Bool"))
        #expect(tabBarBridge.contains("PhoneFeedFilterChromeLayout.resolve("))
        ...
        #expect(tabBarBridge.contains("private var lastAppliedChromeMaterialMode: ChromeMaterialMode?"))
        #expect(tabBarBridge.contains("private var lastPublishedGeometry: TabBarGeometrySnapshot?"))
        #expect(phoneFeedFilterLayout.contains("struct PhoneFeedFilterChromeLayout: Equatable"))
        #expect(phoneFeedFilterLayout.contains("static let pillHeight: CGFloat = 34"))
        #expect(phoneFeedFilterLayout.contains("static let panelHeight: CGFloat = 46"))
        #expect(phoneFeedFilterLayout.contains("preferredPillWidth: CGFloat = minimumPillWidth"))
        #expect(phoneFeedFilterLayout.contains("private static let pillWidth") == false)
        #expect(phoneFeedFilterLayout.contains("let pillFrame: CGRect"))
        #expect(phoneFeedFilterLayout.contains("let panelFrame: CGRect"))
        #expect(phoneFeedFilterLayout.contains("contentIsAtStart") == false)
        #expect(phoneFeedFilterLayout.contains("selectedItemFrame") == false)
        #expect(l10n.contains("static var feedFilter: String"))
        #expect(l10n.contains("static var filtering: String"))
        #expect(l10n.contains("static var filterActivity: String"))
        #expect(l10n.contains("static var filterNovels: String"))
        #expect(l10n.contains("static var filterResults: String"))
        #expect(localizable.contains("\"Filter\""))
        #expect(localizable.contains("\"Filtering\""))
        #expect(localizable.contains("\"Filter activity...\""))
        ...
        #expect(tabBarBridge.contains("if controller.chromeMaterialMode != chromeMaterialMode"))
        ...
        #expect(tabBarBridge.contains("appearance.configureWithOpaqueBackground()") == false)
        #expect(tabBarBridge.contains("appearance.backgroundEffect = nil"))
        #expect(tabBarBridge.contains("UIBlurEffect(style: .systemChromeMaterial)"))
        ...
        #expect(tabBarBridge.contains("tabBar.scrollEdgeAppearance = (usesTransparentBackground || chromeMaterialMode == .plain) ? appearance : nil"))
        ...
        #expect(iPadContentView.contains(".iOSMinimizedSearchToolbar()"))
    ...
    func pixivCollectionsStayNativeAndReachableFromCompactNavigation() throws {
        ...
        #expect(collectionsView.contains("compactDiscoverySelectionMenu") == false)
        ...
        #expect(collectionsView.contains("L10n.openPixivLinkFromClipboard") == false)
        #expect(collectionsView.contains("usesCompactDiscoveryChrome") == false)
        #expect(iPadContentView.contains("pixivCollectionsToolbarItem"))
        #expect(iPadContentView.contains("pixivCollectionsToolbarMenu"))
        #expect(iPadContentView.contains("NativeToolbarMenuButton("))
        #expect(iPadContentView.contains("L10n.pixivCollectionSource"))
        #expect(iPadContentView.contains("IPadToolbarMenuAction.pixivCollectionScope"))
        #expect(iPadContentView.contains("IPadToolbarMenuAction.pixivCollectionTag"))
        #expect(iPadContentView.contains("IPadToolbarMenuAction.openPixivCollectionWeb"))
        #expect(iPadContentView.contains("store.selectPixivCollectionDiscoveryScope(scope)"))
        #expect(iPadContentView.contains("store.selectPixivCollectionDiscoveryTag(tag)"))
        #expect(iPadContentView.contains("store.isPixivWebSessionPresented = true"))
        ...
        #expect(mobileConfiguration.contains("id: \"discovery-pixiv-collections\""))
        #expect(mobileConfiguration.contains("id: \"bookmarks-pixiv-collections\"") == false)
        #expect(mobileConfiguration.contains("PixivRoute.pixivCollectionsLibraryRoutes"))
        ...
        #expect(dashboardModels.contains(".savedPixivCollections") == false)
    ...
    func pixivActivityFeedUsesNativeListOrMasonryAndVisibleRoute() throws {
        ...
        #expect(activityView.contains("isNew: store.pixivActivityNewItemIDs.contains(item.id)"))
        #expect(activityView.contains("OS26LibrarySearchField("))
        #expect(activityView.contains("text: $store.clientFilterQuery"))
        #expect(activityView.contains("placeholder: L10n.filterActivity"))
        #expect(activityView.contains("activityFilterPills"))
        #expect(activityView.contains("FeedFilterClearChip("))
        #expect(activityView.contains("store.setPixivActivityFeedScope(.all)"))
        ...
        #expect(activityView.contains("activityStatusColumn"))
        #expect(activityView.contains("topTrailingNewBadge"))
        #expect(activityView.contains("bottomMetadataOverlay"))
        #expect(activityView.contains("newActivityBadge"))
        ...
        #expect(activityView.contains("activityRefreshResult.newItemCount"))
        #expect(activityView.contains("store.loadMorePixivActivityFeed()"))
        #expect(activityView.contains("store.pixivActivityVisibleItems"))
        #expect(activityView.contains("filteredActivityEmptyState"))
        #expect(activityView.contains("L10n.pixivActivityFilteredEmptyTitle"))
        #expect(activityView.contains("clearActivityTextFilter()"))
        #expect(activityView.contains("store.setPixivActivityKindFilter(.all)"))
        #expect(activityView.contains("usesPhoneCurrentFeedFilterOverlay"))
        ...
        #expect(activityPresentation.contains("enum PixivActivityKindFilter"))
        #expect(activityPresentation.contains("static func filteredItems("))
        #expect(activityPresentation.contains("query: String = \"\""))
        #expect(activityPresentation.contains("static func activitySearchText(for item: PixivActivityItem)"))
        #expect(store.contains("func refreshPixivActivityFeed() async"))
        #expect(store.contains("func loadMorePixivActivityFeed() async"))
        #expect(store.contains("var pixivActivityVisibleItems"))
        #expect(store.contains("func setPixivActivityKindFilter(_ filter: PixivActivityKindFilter)"))
        ...
        #expect(storeCore.contains("var pixivActivityKindFilter"))
        ...
        #expect(contentView.contains("pixivActivityDisplayMenu"))
        #expect(contentView.contains("activityFeedScopeBinding"))
        #expect(contentView.contains("activityKindFilterBinding"))
        #expect(contentView.contains("Picker(L10n.pixivActivityKindFilter, selection: activityKindFilterBinding)"))
        #expect(iPadContentView.contains("PixivActivityFeedView(store: store)"))
        #expect(iPadContentView.contains("showsPixivActivityDisplayPicker(showsSidebarToggle: showsSidebarToggle)"))
        #expect(iPadContentView.contains("pixivActivityDisplayMenu"))
        #expect(iPadContentView.contains("cacheKey: pixivActivityDisplayMenuCacheKey"))
        #expect(iPadContentView.contains("NativeToolbarMenuItem.singleSelectionSubmenu("))
        #expect(iPadContentView.contains("private var pixivActivityDisplayMenuCacheKey: String"))
        #expect(iPadContentView.contains("title: L10n.pixivActivityFeedScope"))
        #expect(iPadContentView.contains("selectedTitle: store.pixivActivityFeedScope.title"))
        #expect(iPadContentView.contains("title: L10n.pixivActivityKindFilter"))
        #expect(iPadContentView.contains("selectedTitle: store.pixivActivityKindFilter.title"))
        #expect(iPadContentView.contains("PixivActivityKindFilter.allCases"))
        #expect(iPadContentView.contains("id: IPadToolbarMenuAction.pixivActivityScope"))
        #expect(iPadContentView.contains("id: IPadToolbarMenuAction.pixivActivityKind"))
        #expect(iPadContentView.contains("store.setPixivActivityFeedScope(scope)"))
        #expect(iPadContentView.contains("store.setPixivActivityKindFilter(filter)"))
        #expect(iPadContentView.contains("IPadToolbarMenuAction.pixivActivityKindFilter(from: id)"))
        ...
        #expect(localizable.contains("\"Activity Type\""))
        #expect(localizable.contains("\"All Activity Types\""))
        #expect(localizable.contains("\"No matching activity\""))
    ...
    func pixivWebSessionConnectionIsExplicitAndAvailableOnAllAppShells() throws {
        ...
        #expect(api.contains("PixivWebPageInspector.activityPageLooksAccessible("))
        #expect(api.contains("L10n.pixivWebSessionNotSignedIn"))
        #expect(api.contains("PixivWebURLBuilder.activityFeedURL()"))
        ...
        #expect(webSessionSheet.contains("PixivWebURLBuilder.activityFeedURL()"))
        ...
        #expect(store.contains("refreshAfterPixivWebSessionChange()"))
        #expect(store.contains("selectedRoute == .pixivActivity"))
    ...
    func iPhonePageChromeKeepsBoardTitlesOutOfContentHeader() throws {
        ...
        #expect(emptyState.contains("if isPhone, Trailing.self == EmptyView.self"))
        #expect(emptyState.contains("Spacer(minLength: 0)"))
        #expect(emptyState.contains("trailing()"))
        ...
        #expect(mobileTopChrome.contains("func mobileToolbarChromeMaterial(syncID: String = \"default\")"))
        #expect(mobileTopChrome.contains("modifier(MobileFloatingTopChromeModifier(syncID: syncID))"))
        #expect(mobileTopChrome.contains("modifier(MobileToolbarChromeMaterialModifier(syncID: syncID))"))
        #expect(mobileTopChrome.contains("struct MobileFloatingTopChromeModifier: ViewModifier"))
        #expect(mobileTopChrome.contains("struct MobileToolbarChromeMaterialModifier: ViewModifier"))
        #expect(mobileTopChrome.contains("@Environment(\\.chromeMaterialMode) private var chromeMaterialMode"))
        #expect(mobileTopChrome.contains(".mobileToolbarChromeMaterial(syncID: syncID)"))
        #expect(mobileTopChrome.contains("if chromeMaterialMode == .liquidGlass"))
        #expect(mobileTopChrome.contains(".toolbarBackground(.hidden, for: .navigationBar)"))
        #expect(mobileTopChrome.contains(".toolbarBackground(.ultraThinMaterial, for: .navigationBar)"))
        #expect(mobileTopChrome.contains("Color(uiColor: .systemBackground).opacity(0.97)") == false)
        #expect(mobileTopChrome.contains(".toolbarBackground(.visible, for: .navigationBar)"))
        #expect(mobileTopChrome.contains("struct NavigationBarChromeBridge: UIViewControllerRepresentable"))
        #expect(mobileTopChrome.contains("let chromeMaterialMode: ChromeMaterialMode"))
        #expect(mobileTopChrome.contains("NavigationBarChromeBridge(\n                    chromeMaterialMode: chromeMaterialMode"))
        ...
        #expect(mobileTopChrome.contains("appearance.configureWithOpaqueBackground()") == false)
        #expect(mobileTopChrome.contains("appearance.backgroundEffect = nil"))
        #expect(mobileTopChrome.contains("appearance.backgroundColor = .clear"))
        #expect(mobileTopChrome.contains("UIBlurEffect(style: .systemChromeMaterial)"))
        #expect(mobileTopChrome.contains("navigationController.navigationBar.isTranslucent = true"))
        #expect(mobileTopChrome.contains("navigationController.navigationBar.isTranslucent = false") == false)
    ...
    func translationUIStringsStayCatalogBacked() throws {
        ...
        #expect(l10n.contains(#"novelTranslationUnsupportedLanguagePair: String { text("Apple Translation does not support this language pair.") }"#))
        #expect(l10n.contains(#"novelTranslationModelNotInstalled: String { text("Apple Translation needs a language model. Approve the system download, then try again.") }"#))
        #expect(l10n.contains(#"novelTranslationCancelled: String { text("Translation was cancelled.") }"#))
        ...
        #expect(localizable.contains(#""Apple Translation does not support this language pair.""#))
        #expect(localizable.contains(#""Apple Translation needs a language model. Approve the system download, then try again.""#))
        #expect(localizable.contains(#""Translation was cancelled.""#))
    ...
    @Test("macOS feed uses system sidebar toggle and lifts artwork navigation")
    func macOSFeedUsesSystemSidebarToggleAndLiftsArtworkNavigation() throws {
        let root = try packageRoot()
        let contentView = try String(
            contentsOf: root.appending(path: "Sources/KeiPix/Views/ContentView.swift"),
            encoding: .utf8
        ...
        #expect(contentView.contains("private var showsArtworkNavigationControls: Bool"))
        #expect(contentView.contains(".toolbar(removing: .sidebarToggle)") == false)
        #expect(contentView.contains("private func toggleSidebar()") == false)
        #expect(contentView.contains("Label(sidebarVisible ? L10n.hideSidebar : L10n.showSidebar, systemImage: \"sidebar.leading\")") == false)
    ...
    func macOSToolbarUsesGroupedAppKitChrome() throws {
        ...
        #expect(contentView.contains("Section(L10n.imageQualityTierSection)"))
        #expect(contentView.contains("Label(L10n.imageQualityTierSection, systemImage: imageQualityMenuSystemImage)"))
        #expect(contentView.contains("qualityTierMenu(") == false)
        #expect(contentView.contains("setArtworkImageQualityTier("))
        #expect(contentView.contains("L10n.feedPreviewQuality") == false)
        #expect(contentView.contains("L10n.illustDetailQuality") == false)
        #expect(contentView.contains("L10n.mangaDetailQuality") == false)
    ...
    func macOSLaunchSizingClampsRestoredNarrowWindows() throws {
        ...
        #expect(sidebarView.contains("static let macOS = SidebarColumnWidth(ideal: 264)"))
        #expect(sidebarView.contains("static let iPadOS = SidebarColumnWidth(ideal: 224)"))
        ...
        #expect(sidebarView.contains(".navigationTitle(\"KeiPix\")"))
        ...
        #expect(sidebarView.contains(".frame(\n            minWidth: columnWidth.min") == false)
        #expect(sidebarView.contains("idealWidth: columnWidth.ideal") == false)
        #expect(sidebarView.contains("maxWidth: columnWidth.max") == false)
        #expect(sidebarView.contains(".navigationSplitViewColumnWidth(ideal: columnWidth.ideal)"))
        ...
        #expect(accountIdentityMenu.contains("showIdentity ? 30 : 34"))
    ...
    func feedAndLibraryStatusChromeUsesCompactCounts() throws {
        ...
        #expect(mangaWatchlist.contains("if hasActiveWatchlistFilterState == false"))
    ...
    func settingsWorkspaceUsesAdaptiveOS26Layout() throws {
        ...
        #expect(generalSettings.contains("L10n.illustDetailQuality") == false)
        #expect(generalSettings.contains("L10n.mangaDetailQuality") == false)
        ...
        #expect(settingsSurface.contains("struct OS26SettingsInlineOptionPicker<Option: Identifiable & Hashable, RowLabel: View>: View"))
        #expect(settingsSurface.contains("OS26SettingsInlineOptionPicker("))
        ...
        let controlRowRange = try #require(settingsSurface.range(of: "struct OS26SettingsControlRow<Accessory: View>: View"))
        let toggleRowRange = try #require(settingsSurface.range(of: "struct OS26SettingsToggleRow: View"))
        let controlRowSource = settingsSurface[controlRowRange.lowerBound..<toggleRowRange.lowerBound]
        #expect(controlRowSource.contains("ViewThatFits(in: .horizontal)") == false)
        #expect(controlRowSource.contains("private var accessoryWidth: CGFloat"))
        let rowLabelRange = try #require(settingsSurface.range(of: "struct OS26SettingsRowLabel: View"))
        let controlRowStartRange = try #require(settingsSurface.range(of: "struct OS26SettingsControlRow<Accessory: View>: View"))
        let rowLabelSource = settingsSurface[rowLabelRange.lowerBound..<controlRowStartRange.lowerBound]
        #expect(rowLabelSource.contains("Text(detail)"))
        #expect(rowLabelSource.contains(".lineLimit(2, reservesSpace: false)") == false)
        #expect(rowLabelSource.contains(".fixedSize(horizontal: false, vertical: true)"))
        ...
        #expect(readingSettings.contains("tone: .accent"))
        #expect(safetySettings.contains("OS26SettingsToggleRow("))
        #expect(safetySettings.contains("tone: .danger"))
        #expect(safetySettings.contains("tone: .safety"))
        #expect(safetySettings.contains("tone: .privacy"))
        #expect(generalSettings.contains("tone: .network"))
        #expect(downloadsSettings.contains("tone: .downloads"))
    ...
    func galleryFeedLayoutsUseNativeCollectionBridge() throws {
        ...
        #expect(artworkCard.contains("struct ArtworkCardOverlayContrast"))
        #expect(artworkCard.contains("colors: overlayContrast.scrimColors"))
        #expect(artworkCard.contains(".foregroundStyle(overlayContrast.titleColor)"))
        #expect(artworkCard.contains(".foregroundStyle(overlayContrast.secondaryColor)"))
        #expect(artworkCard.contains(".foregroundStyle(overlayContrast.metricsColor)"))
        #expect(artworkCard.contains("color: overlayContrast.textShadowColor"))
        ...
        #expect(nativeCollection.contains("NativeCollectionLayoutMetrics.artworkCards"))
    ...
    func novelReaderTextPagesUseNativeTextKitBridges() throws {
        ...
        #expect(readerView.contains(".onChange(of: novelStore.loadedNovelTokens.count)"))
        ...
        #expect(readerView.contains("AppleTranslationPreparationPolicy.shouldPrepareSession(sourceLanguage: translationSourceLanguage)"))
        ...
        #expect(readerView.contains("translationStatusBadge(translationStatusMessage, allowsMultiline: true)"))
        #expect(readerView.contains("private func translationStatusBadge(_ message: String, allowsMultiline: Bool)"))
        #expect(readerView.contains("ViewThatFits(in: .horizontal) {\n                    Label(translationStatusMessage") == false)
        ...
        #expect(appleTranslationSupport.contains("enum AppleTranslationPreparationPolicy"))
        #expect(appleTranslationSupport.contains("static func shouldPrepareSession(sourceLanguage: Locale.Language?) -> Bool"))
    ...
    func artworkReaderNativeScrollViewportsKeepHeightStable() throws {
        ...
        #expect(iPadImageScrollView.contains("prepareImageViewForReplacement()"))
        #expect(iPadImageScrollView.contains("scrollView.setZoomScale(1, animated: false)"))
        #expect(iPadImageScrollView.contains("imageView.transform = .identity"))
        ...
        #expect(imageScrollView.contains("prepareImageViewForReplacement()"))
        #expect(imageScrollView.contains("scrollView.magnification = 1"))
    ...
    func downloadQueueUsesNativeMasonryCollectionContainer() throws {
        ...
        #expect(queueView.contains("@State private var isSearchPresented = false"))
        #expect(queueView.contains("OS26LibrarySearchField("))
        #expect(queueView.contains("FeedFilterClearChip("))
        #expect(queueView.contains("private var filterTriggerPill: some View"))
        #expect(queueView.contains("private var showsDownloadSearchField: Bool"))
        #expect(queueView.contains("latestCompletedLine\n            metricStrip\n            filterControls") == false)
        #expect(queueView.contains("L10n.filterDownloads"))
        #expect(queueView.contains("showsTitle: false"))
        #expect(queueView.contains("if showsTitle"))
        #expect(queueView.contains("private var summaryIconTopPadding: CGFloat"))
        #expect(queueView.contains("downloadRelativeTimeText(for: completedAt)"))
        #expect(queueView.contains("Text(completedAt, style: .relative)") == false)
        ...
        #expect(iPadContentView.contains("private func downloadQueueToolbarItem(showsSidebarToggle: Bool) -> some ToolbarContent"))
        #expect(iPadContentView.contains("systemImage: \"arrow.down.circle\""))
        #expect(iPadContentView.contains("accessibilityLabel: L10n.downloadActions"))
    ...
    func mangaWatchlistUsesNativeAdaptiveGrid() throws {
        ...
        #expect(watchlist.contains("private var watchlistRootWithPageHeader: some View"))
        #expect(watchlist.contains("if usesPhoneWatchlistFilterPill"))
        #expect(watchlist.contains(".mobilePageFilter(mobileWatchlistPageFilterSnapshot)"))
        #expect(watchlist.contains("@State private var watchlistSelection = MangaWatchlistSelection()"))
        #expect(watchlist.contains("MangaWatchlistActionsMenu("))
        #expect(watchlist.contains("MangaWatchlistSelectionFloatingActions("))
        #expect(watchlist.contains("NativeToolbarMenuButton("))
        #expect(watchlist.contains("cacheKey: nativeActionsMenuCacheKey"))
        #expect(watchlist.contains("NativeCollectionLayoutMetrics.bottomTabInformationCards"))
    ...
    func mangaWatchlistKeepsContentPrimaryAndActionsCompact() throws {
        ...
        #expect(watchlist.contains("watchlistCoverCard"))
        #expect(watchlist.contains("coverMetadataOverlay"))
        #expect(watchlist.contains("contentMode: .fit"))
        #expect(watchlist.contains("mangaWatchlistContextMenu"))
        #expect(watchlist.contains("ShareLink(item: url)"))
        #expect(watchlist.contains("PasteboardWriter.copy(url.absoluteString)"))
        #expect(watchlist.contains("mangaWatchlistPrimaryActions") == false)
        #expect(watchlist.contains("mangaWatchlistAuxiliaryActions") == false)
        #expect(watchlist.contains("mangaWatchlistSeriesLink") == false)
        #expect(watchlist.contains("Link(destination: url)") == false)
        #expect(watchlist.contains("Label(L10n.openLatestArtwork, systemImage: \"arrow.right.circle\")") == false)
        #expect(watchlist.contains("Button {\n                openInPixiv(url)"))
    ...
    func libraryManagementSurfacesUseOS26NativeSearchAndGlassActions() throws {
        ...
        #expect(downloads.contains("@State private var isSearchPresented = false"), "DownloadQueueView.swift should open download search from explicit state")
        #expect(downloads.contains("OS26LibrarySearchField("), "DownloadQueueView.swift should use the shared native library search field in the overview card")
        #expect(downloads.contains("L10n.filterDownloads"), "DownloadQueueView.swift should present download search as local filtering")
        #expect(downloads.contains("private var showsDownloadSearchField: Bool"), "DownloadQueueView.swift should keep the search field opt-in instead of always visible")
    ...
    func libraryManagementChromeKeepsPageActionsCompact() throws {
        ...
        #expect(downloads.contains("OS26LibrarySearchField("))
    ...
    func creatorListSearchMenuAndDropUseNativeP2Bridges() throws {
        ...
        #expect(userPreviewList.contains("@State private var creatorSelection = CreatorListSelection()"))
        #expect(userPreviewList.contains("CreatorSelectionFloatingActions("))
        #expect(userPreviewList.contains("selectedCreatorPreviews.filter"))
        #expect(userPreviewList.contains("private func copySelectedCreatorLinks()"))
        #expect(userPreviewList.contains("private func copyVisibleCreatorLinks()") == false)
        ...
        #expect(userPreviewList.contains("selectRoute: { store.select($0) }"))
        #expect(optionsMenuSource.contains("if mode.usesRestrictPicker"))
        #expect(optionsMenuSource.contains("let selectedRoute: PixivRoute?"))
        #expect(optionsMenuSource.contains("#if os(iOS)\n        NativeToolbarMenuButton("))
        #expect(optionsMenuSource.contains("private var nativeViewOptionsMenu: NativeToolbarMenu"))
        #expect(optionsMenuSource.contains("cacheKey: nativeViewOptionsMenuCacheKey"))
        #expect(optionsMenuSource.contains("NativeToolbarMenuItem.singleSelectionSubmenu("))
        #expect(optionsMenuSource.contains("CreatorListViewOptionsAction.route"))
        #expect(optionsMenuSource.contains("private func handleNativeViewOptionsAction(_ id: String)"))
        #expect(optionsMenuSource.contains("private enum CreatorListViewOptionsAction"))
        #expect(optionsMenuSource.contains("private var swiftUIViewOptionsMenu: some View"))
        #expect(optionsMenuSource.contains("title: L10n.followingCreators"))
        #expect(optionsMenuSource.contains("selection: $restrict"))
        #expect(optionsMenuSource.contains("Section(L10n.viewOptions)"))
        #expect(optionsMenuSource.contains("creatorListPickerMenu("))
        #expect(optionsMenuSource.contains("currentValueTitle: restrict.title"))
        #expect(optionsMenuSource.contains("currentValueTitle: layoutMode.title"))
        #expect(optionsMenuSource.contains("currentValueTitle: creatorFilter.title"))
        #expect(optionsMenuSource.contains("currentValueTitle: creatorSort.title"))
        #expect(optionsMenuSource.contains(".menuOrder(.fixed)"))
        #expect(creatorComponents.contains("struct CreatorSelectionFloatingActions: View"))
        #expect(creatorComponents.contains("let startSelection: () -> Void"))
        #expect(creatorComponents.contains("copyVisibleCreatorLinks") == false)
        #expect(userPreviewList.contains("private func clearCreatorSearch()"))
        #expect(userPreviewList.contains("showsRestrictPicker: false"))
        #expect(creatorCard.contains("let isSelectionMode: Bool"))
        #expect(creatorCard.contains("private var selectionActionChip: some View"))
        #expect(creatorCard.contains("private var followControlTitle: String"))
        #expect(creatorCard.contains("title: followControlTitle"))
        #expect(creatorCard.contains("@Environment(\\.horizontalSizeClass) private var horizontalSizeClass"))
        #expect(creatorCard.contains("private var usesIconOnlyActionLabels: Bool"))
        #expect(creatorCard.contains("if horizontalSizeClass == .compact"))
        #expect(creatorCard.contains("return expandedPreview == false"))
        #expect(creatorCard.contains("if usesIconOnlyActionLabels {\n                    compactCommandRail"))
        #expect(creatorCard.contains("private var compactCommandRail: some View"))
        #expect(creatorCard.contains("GlassEffectContainer(spacing: 6)"))
        #expect(creatorCard.contains("private var headerActionControls: some View"))
        #expect(creatorCard.contains("private var followControlProfile: CreatorCardFollowControlProfile"))
        #expect(creatorCard.contains("private struct CreatorCardFollowControlProfile"))
        #expect(creatorCard.contains("private var pinActionChip: some View"))
        #expect(creatorCard.contains("private var pinControlProfile: CreatorCardPinControlProfile"))
        ...
        #expect(compactRailSource.contains("overflowMenu") == false)
        #expect(creatorCard.contains("private var feedbackActionChip: some View"))
        #expect(creatorCard.contains("private var muteCreatorActionChip: some View"))
        #expect(creatorCard.contains("systemImage: \"exclamationmark.bubble\""))
        #expect(creatorCard.contains("systemImage: \"eye.slash\""))
        #expect(creatorCard.contains(".foregroundStyle(.red)"))
        #expect(creatorCard.contains("fullWidthNavigationButtons(displayStyle: .titleAndIcon)"))
        #expect(creatorCard.contains("expandedCommandRailContent(displayStyle: .iconOnly)"))
        #expect(creatorCard.contains("actionRailContent(pixivDisplayStyle: .iconOnly)"))
        ...
        #expect(enhancedMenu.contains("case startCreatorSelection"))
        #expect(enhancedMenu.contains("case selectAllVisibleCreators"))
        #expect(enhancedMenu.contains("case clearCreatorSelection"))
    ...
    func searchClearActionsResetRouteStateAndLoadingSurfacesStayStable() throws {
        ...
        #expect(macContentView.contains(".iOSMinimizedSearchToolbar()") == false)
    ...
    func searchSurfacesUseNativeFieldsAndOS26GlassChrome() throws {
        ...
        #expect(searchWorkspace.contains("SearchWorkspaceActionsMenu("))
        #expect(searchWorkspace.contains("NativeToolbarMenuButton("))
        #expect(searchWorkspace.contains("GalleryView("))
        ...
        #expect(searchWorkspace.contains("private var searchHeaderActionRail: some View"))
        #expect(searchWorkspace.contains("searchLandingActionsSection"))
        #expect(searchWorkspace.contains("searchPrimaryActionGroup") == false)
        #expect(searchWorkspace.contains("searchTargetMenu") == false)
        #expect(searchWorkspace.contains("searchUtilityActionRail") == false)
        #expect(searchWorkspace.contains("compactSearchUtilityActionRail") == false)
        #expect(searchWorkspace.contains("searchUtilityMenu") == false)
        #expect(searchWorkspace.contains("searchQuickActionsSection") == false)
        ...
        #expect(searchWorkspace.contains(".os26GlassButton(prominent: isProminent)"))
    ...
    func creatorCardsUseAdaptiveOS26GlassActions() throws {
        ...
        #expect(creatorCard.contains("NativeCollectionLayoutMetrics.compactCreatorPreviewAspect"))
        #expect(creatorCard.contains("previewStripAspect(for:") == false)
