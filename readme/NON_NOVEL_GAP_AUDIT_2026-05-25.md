...
| Multi-page reader | Continuous manga reading, zoom page, page navigation, swipe/trackpad patterns. | Per-page aspect ratio, continuous/single/index modes, trackpad swipe, magnify, smart zoom, keyboard, reader window. | Add optional full-screen reader window preset, page-range jump overlay, and persisted reader mode per work type. | P1 |
...
| Downloads | Queue, concurrent tasks, naming templates, batch download, local library. | Queue, filters/search/sort, template placeholders, visible batch actions, page-range multi-P downloads, local previews, Finder actions. | Add retry-all-failed with backoff and optional auto-bookmark-after-download parity. | P1 |
...
| Reader | 91% | Strong macOS trackpad and multi-P support. | Add full-screen preset and persisted reading defaults. |
| Artwork detail/social | 90% | Core actions and comments work. | Add comment stamps only after endpoint verification. |
| Downloads | 94% | Strong queue, page-range multi-P downloads, and local library. | Add retry-all-failed backoff and optional auto-bookmark-after-download. |
...
| 5 | Deepen reader/download parity | Add full-screen reader preset, retry-all-failed with backoff, and optional auto-bookmark-after-download. | 50P+ manga can be read and downloaded by page range; failed downloads can recover in one action. |
