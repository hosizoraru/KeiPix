...
| Artwork gallery | Pixez/Pixes use masonry/staggered grids and special handling for wide images. | Custom SwiftUI `MasonryLayout`, fixed two/three/auto/compact modes, badges, selected state, context menus, and `script/capture_visual_qa.sh` for window-scoped visual evidence. | Capture and review repeatable screenshots for trending tags, Pixivision cards, wide/tall mixed feeds, and narrow window states. | P0 QA |
...
| Multi-page reader | Continuous manga reading, zoom page, page navigation, swipe/trackpad patterns. | Per-page aspect ratio, continuous/single/index modes, persisted mode defaults by work type, trackpad swipe, magnify, smart zoom, keyboard, reader window, full-screen focus preset, and page-jump overlay. | Add optional per-artwork last-page restoration. | P2 |
...
| Gallery / masonry | 88% | Core layout exists and window-scoped visual QA capture is available. | Capture hot tags/Pixivision/wide-feed/narrow-window screenshots and review them. |
| Reader | 97% | Strong macOS trackpad, persisted reading defaults, full-screen focus preset, page jump, and multi-P support. | Add optional per-artwork last-page restoration. |
...
| 5 | Deepen reader/download parity | Add optional auto-bookmark-after-download and per-artwork reader resume. | 50P+ manga can be read, jumped through, focused full-screen, and downloaded by page range; failed downloads recover in one staggered action. |
