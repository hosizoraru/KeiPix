...
SwiftPM 脚本默认只构建 `arm64`；需要诊断其它架构时可临时设置
`KEIPIX_SWIFTPM_ARCH=<arch>`，但正式 KeiPix 产物不再生成 x86_64 slice。
...
swift test --arch arm64 --filter GallerySelectionTests
```
...
- `swift build --arch arm64 --jobs <logical-cpu-count>`
- `swift test --arch arm64 --parallel --jobs <logical-cpu-count>`
