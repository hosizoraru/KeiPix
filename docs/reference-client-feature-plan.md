...
| Local filtering DSL | Partial | Pixeval has a rich work filter language for title, author, tag, ratio, date, R-18, AI, GIF, bookmark, and index checks. | KeiPix has `ClientFilterDSL`, compact bottom-bar in-feed filtering, and a completed first model foundation for native advanced filter criteria. | Keep iPhone lightweight; build the visible advanced editor for iPad landscape/macOS on top of the structured draft model instead of exposing raw DSL as the default UI. |
...
| P1 | Partial | Advanced local filter editor | Pixeval's filter language can target title, author, tag, ratio, date, R-18/R-18G, AI, GIF, bookmark count, and page index. | KeiPix now has a structured draft model that emits existing/extended `ClientFilterDSL` tokens for tag, title, author, bookmark/page/view ranges, R-18/R-18G, AI, GIF/ugoira, bookmarked state, and artwork ratio; the visible native editor surface is still pending. | Keep iPhone simple; build a token/menu-based editor for iPad landscape and macOS instead of making the raw DSL the default UI. |
