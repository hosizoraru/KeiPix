...
| Remote search options | Pixeval exposes `/v1/search/options`. | KeiPix now decodes and caches Pixiv remote search metadata, and uses server-provided bookmark ranges as search filter presets while keeping the existing native search options as the fallback baseline. | Continue by mapping safe language/tool/genre metadata into compact iPhone menus and fuller iPad/macOS filter panes. |
...
| 6 | Search options | Fetch and cache `/v1/search/options`; first UI slice maps server-provided bookmark ranges into the minimum/maximum bookmark preset menus. | Parser/model tests, search option tests, and search UI screenshots when the visible pane changes further. |
...
| Remote search options | In progress | Parser, endpoint URL, API fetch, store cache hook, and bookmark range preset mapping are wired; language/tool/genre metadata remains a later search-polish slice. |
