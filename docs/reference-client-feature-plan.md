...
| Remote search options | Pixeval exposes `/v1/search/options`, and Pixeval's novel search path uses the same match/sort/AI/date vocabulary plus novel-specific language, genre, and length knobs. | KeiPix now decodes and caches Pixiv remote search metadata, uses server-provided bookmark ranges as search filter presets, and routes existing search filters into novel search query/local filtering. | Continue by mapping safe language/tool/genre/length metadata into compact iPhone menus and fuller iPad/macOS filter panes. |
...
| 6 | Search options | Fetch and cache `/v1/search/options`; first UI slice maps server-provided bookmark ranges into the minimum/maximum bookmark preset menus, and the second slice makes novel search honor the shared search filters. | Parser/model tests, search option tests, novel query/filter tests, and search UI screenshots when the visible pane changes further. |
...
| Remote search options | In progress | Parser, endpoint URL, API fetch, store cache hook, bookmark range preset mapping, and novel search filter/query wiring are done; language/tool/genre/length metadata remains a later search-polish slice. |
