...
| Reader tools | Partial | Pixeval has image viewer modes, page preview slider, rotate/mirror tools. | KeiPix has native readers, paging, continuous/double page, zoom, basic slider, native macOS/iPad landscape filmstrip rail, and standalone reader rotation controls; mirror/transform controls still need to be unified. | Treat the native thumbnail rail as complete; unify rotate/mirror in the reader command model as the next explicit reader-tools slice. |
