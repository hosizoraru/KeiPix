...
| Activity feed | Planned | Pixeval parses Pixiv Web stacc activity feed. | KeiPix has content feeds, not a social activity stream. | Medium priority; Web-backed, parser-first, failure-tolerant. |
...
| Reader tools | Partial | Pixeval has image viewer modes, page preview slider, rotate/mirror tools. | KeiPix has native readers, paging, continuous/double page, zoom, basic slider, a filmstrip presentation model, and standalone reader rotation controls; the online reader still needs the native filmstrip rail to be verified and mirror/transform controls to be unified. | Finish the native thumbnail rail first, then unify rotate/mirror in the reader command model. |
...
| Novel follow and bookmarks | Partial | pixez-flutter exposes novel follow, public/private novel bookmarks, novel ranking, novel watchlist, and series pages as one novel workflow. | KeiPix has novel recommended, ranking, search, series, watchlist, bookmark actions, series chapter sheets, and related navigation, but latest-novel and novel bookmark tag suggestions still need first-class wiring. | Continue unifying novel surfaces with compact iPhone menus and fuller iPad landscape/macOS panes. |
| Reader page preview | In progress | Pixeval has a page preview slider/filmstrip for image readers. | KeiPix has native readers, page restore, paging modes, and a tested filmstrip presentation model; a native collection-backed rail exists in the worktree but still needs final build/visual verification before Done. | Complete the rail slice on iPad landscape and macOS; keep iPhone transient and unobtrusive. |
...
| Local filtering DSL | Partial | Pixeval has a rich work filter language for title, author, tag, ratio, date, R-18, AI, GIF, bookmark, and index checks. | KeiPix has `ClientFilterDSL` plus bottom-bar in-feed filtering. | Keep iPhone lightweight; consider an advanced native editor for iPad landscape/macOS later. |
...
| --- | --- | --- | --- | --- | --- |
| P0 | Planned | Latest novel feed | Pixeval exposes `NovelNewEngine` and the app API path `/v1/novel/new`. | Novel routes cover recommended, following, search, rankings, watchlist, series, bookmarks, related, and MyPixiv, but there is no dedicated latest-novel route. | Add a `.novelLatest` route backed by `/v1/novel/new`; keep iPhone entry compact inside the novel menu and expose fuller sidebar/dashboard placement on iPad landscape and macOS. |
| P0 | Planned | Novel bookmark tag suggestions | Pixeval exposes `/v1/user/bookmark-tags/novel`. | Artwork bookmark sheets have tag suggestions, but novel bookmarks do not have the novel-specific tag endpoint wired. | Add a novel bookmark tag page model/API/store hook and use it in the novel bookmark editor without sharing artwork-only tag state. |
| P0 | In progress | Native reader filmstrip rail | Pixeval has a page preview slider/filmstrip for image readers. | The presentation model slice is done; the native collection-backed rail is currently in the worktree and has focused boundary/model tests, but still needs final full-test/build/visual verification. | Keep `NativeReaderFilmstripCollectionView` under `Support` with UIKit/AppKit collection ownership; mount it only for landscape iPad and macOS when `ReaderFilmstripPresentation` is visible. |
| P1 | Planned | Cross-page batch download | pixes can fetch successive `nextUrl` pages up to a user-selected maximum before enqueueing downloads. | KeiPix supports selected/loaded batch downloads and a strong queue, but cannot yet intentionally gather the next N remote items from the current feed. | Add a confirmation sheet with a maximum count, page-request estimate, cancellation, and duplicate skipping; keep remote fetching explicit and bounded. |
| P1 | Planned | Pixiv activity feed | Pixeval parses Pixiv Web stacc activity for followed users' posts, bookmarks, and favorite-user actions. | KeiPix has content feeds but no social activity stream. | Build a Web-backed parser fixture first; expose as a separate discovery route only after signed-in failure states are robust. |
| P1 | Partial | Reader rotate/mirror tools | Pixeval image viewer includes rotation and mirror tools. | Standalone reader has rotation controls, but online/detail reader transforms and mirror state are not yet a shared command model. | Add transforms to the reader command model first, then place controls in the iPad landscape/macOS tool menu and keep iPhone inside More. |
| P1 | Planned | Advanced local filter editor | Pixeval's filter language can target title, author, tag, ratio, date, R-18/R-18G, AI, GIF, bookmark count, and page index. | KeiPix has `ClientFilterDSL` and compact bottom-bar filtering, but no native advanced editor for wider layouts. | Keep iPhone simple; build a token/menu-based editor for iPad landscape and macOS instead of making the raw DSL the default UI. |
...
| 7 | In progress | Reader/export polish | Completed novel HTML export and filmstrip presentation model; native reader filmstrip rail is the active unverified slice. | Export formatter tests passed earlier; final native rail slice still needs full tests/builds and visual evidence. |
...
| Reader page preview | In progress | Reader filmstrip visibility now resolves from page count, platform, and available reader size; phone and portrait iPad stay hidden, while landscape iPad and macOS expose a leading-rail contract with stable artwork/page item IDs. A native collection bridge is in the worktree, but it is not Done until final tests/builds and visual evidence pass. |
